...
public class McpTools {
    ...
    public String setSchedules(
            @McpToolParam(description = "설정할 날짜를 입력합니다. yyyy.MM.dd. 의 형식으로 입력하여야 합니다. (예) 2025.07.24.") String date,
            @McpToolParam(description = "일정을 JSON배열 형식으로 입력합니다. 일정이 없다면 빈 배열([])을 입력하며, 일정이 있을 경우 배열 안에 JSON 객체({})가 들어가며, 들어가는 객체는 content라는 필드를 가집니다. content필드는 해당 일정의 이름을 의미합니다. 예를 들어, '학원 가기' 일정을 넣으려면 [{\"content\":\"학원 가기\"}] 처럼 값을 삽입합니다.") String schedules
    ) {
        JSONObject obj = new JSONObject();
        obj.put("date", date);
